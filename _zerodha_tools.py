...
class ZerodhaTools:
    ...
    def _save_active_trades(self):
        ...
        # Order matters: EXITS first, then ENTRIES. A reverse trade writes
        # both an exit (old leg) and an entry (new leg). If we consumed
        # entries first, the entry dedup would see the still-present old
        # leg and skip the new one — leaving paper_positions partially
        # synced and causing positions to silently vanish on rapid reverse
        # clicks (PE⇔CE oscillation or equity LONG↔SHORT flip).
    ...
    def _consume_manual_entry_signals(self):
        ...
        try:
            ...
            for pos in entries:
                sym = pos.get('symbol', '')
                new_side = (pos.get('side') or pos.get('direction') or '').upper()
                # Side-aware dedup: a same-symbol position with the SAME side is
                # a true duplicate and we skip it. A same-symbol position with
                # the OPPOSITE side is a manual reverse — we replace the old
                # row in place rather than dropping the new entry. Without this
                # equity reverse (same symbol, flipped direction) silently
                # vanishes because the entry is dedup'd against the still-open
                # old position that the exit consumer is about to remove.
                dup_idx = None
                opp_idx = None
                for _i, _t in enumerate(self.paper_positions):
                    if _t.get('symbol') != sym or _t.get('status', 'OPEN') != 'OPEN':
                        continue
                    _ex_side = (_t.get('side') or _t.get('direction') or '').upper()
                    if new_side and _ex_side and new_side == _ex_side:
                        dup_idx = _i
                        break
                    if new_side and _ex_side and new_side != _ex_side:
                        opp_idx = _i
                ...
                if dup_idx is not None:
                    print(f"   📰 Manual entry SKIP (duplicate same-side): {sym}")
                    continue
                if opp_idx is not None:
                    # Reverse — replace old leg with new leg in place
                    pos['status'] = 'OPEN'
                    self.paper_positions[opp_idx] = pos
                    added.append(sym)
                    print(f"   📰 Manual entry REPLACED (reverse): {sym} qty={pos.get('quantity')} @ ₹{pos.get('avg_price', 0):.2f}")
