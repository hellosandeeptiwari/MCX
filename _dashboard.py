...
# ── Hardened broker-order helper for LIVE dashboard actions ────────────
# Mirrors the bot's `_place_order_autoslice` so manual Exit / Reverse / +1
# orders fired from the UI get the same protections the bot orders enjoy:
#   • market_protection=5  → mandatory for MARKET/SL-M (Kite Apr 2026)
#   • autoslice=true       → auto-splits orders exceeding freeze qty
#   • 350ms rate throttle  → respects Kite's 10/sec orders cap
#   • 429 retry w/ backoff → survives transient throttle spikes
_LAST_DASH_ORDER_TS = 0.0

def _dashboard_place_order(kite, **kwargs):
    """Drop-in replacement for `kite.place_order(...)` with bot-grade safety."""
    import time as _time
    global _LAST_DASH_ORDER_TS

    # 1) Enforce market_protection=5 for MARKET / SL-M (Kite mandate)
    ...
    # 2) Throttle: 350ms min gap between manual orders
    elapsed = _time.time() - _LAST_DASH_ORDER_TS
    if elapsed < 0.35:
        _time.sleep(0.35 - elapsed)

    # 3) Patch the SDK route to request server-side autoslice
    use_autoslice = hasattr(kite, '_routes')
    orig_route = None
    if use_autoslice:
        try:
            orig_route = kite._routes.get("orders.place", "/orders/{variety}")
            if "autoslice" not in orig_route:
                kite._routes["orders.place"] = "/orders/{variety}?autoslice=true"
        except Exception:
            use_autoslice = False

    try:
        # 4) Up to 3 attempts on 429 / rate-limit errors
        for attempt in range(3):
            try:
                result = kite.place_order(**kwargs)
                _LAST_DASH_ORDER_TS = _time.time()
                return result
            except Exception as e:
                err_str = str(e)
                if (('429' in err_str) or ('Too Many Requests' in err_str)
                    ...
                    delay = 1.0 * (2 ** attempt)
                    print(f"   ⚠️ Dashboard order rate-limited, retry in {delay}s ({attempt+1}/3)")
                    _time.sleep(delay)
                    continue
                raise
    finally:
        if use_autoslice and orig_route is not None:
            try:
                kite._routes["orders.place"] = orig_route
            except Exception:
                pass
...
def reverse_trade():
    ...
    try:
        ...
        #
        # CRITICAL: option reverse flips PE↔CE so the *new* row has a
        # different symbol than the *old* row. A naive per-symbol cooldown
        # is bypassed when the user clicks Reverse on the freshly-flipped
        # leg. We therefore lock BOTH the source symbol and the flipped
        # symbol so a click on either side blocks rapid back-and-forth.
        ...
        # Compute the flipped symbol so we can apply the cooldown to BOTH legs
        _flip_sym = symbol
        try:
            if is_option and option_type in ('CE', 'PE'):
                _orig = 'PE' if option_type == 'CE' else 'CE'
                _flip_sym = re.sub(rf'{_orig}$', option_type, symbol)
        except Exception:
            _flip_sym = symbol

        _last_a = _REVERSE_LAST_TS.get(symbol, 0.0)
        _last_b = _REVERSE_LAST_TS.get(_flip_sym, 0.0)
        _last = max(_last_a, _last_b)
        ...
        # Stamp BOTH legs so the next click on either is blocked for the cooldown
        _REVERSE_LAST_TS[symbol] = _now
        _REVERSE_LAST_TS[_flip_sym] = _now
        try:
...
def _reverse_trade_impl(data, symbol, underlying, direction, quantity,
    ...
    try:
        ...
        if target_pos:
            ...
            # ── LIVE MODE: place real exit order synchronously so the user
            #    isn't waiting on the bot's next loop tick. Mirrors exit_trade.
            live_rev_exit_placed = False
            if not PAPER_MODE:
                try:
                    _kite = _get_dashboard_kite()
                    if _kite:
                        # Cancel pending SL-M first if present
                        _slo = target_pos.get('sl_order_id')
                        if _slo and not str(_slo).startswith('PAPER_'):
                            try:
                                _kite.cancel_order(variety='regular', order_id=_slo)
                            except Exception:
                                pass
                        _exch, _tsym = symbol.split(':') if ':' in symbol else ('NSE', symbol)
                        _tx = _kite.TRANSACTION_TYPE_SELL if side in ('BUY', 'LONG') else _kite.TRANSACTION_TYPE_BUY
                        _oid = _dashboard_place_order(
                            _kite,
                            ...
                            quantity=exit_qty,
                            product=_kite.PRODUCT_MIS,
                            order_type=_kite.ORDER_TYPE_MARKET,
                            validity=_kite.VALIDITY_DAY,
                            tag='TITAN_REVERSE_EXIT',
                        )
                        # Use actual fill price if available
                        try:
                            _fp = _get_completed_order_fill_price(_kite, _oid)
                            if _fp > 0:
                                exit_price = _fp
                                if side in ('BUY', 'LONG'):
                                    exit_pnl = (exit_price - entry_price) * exit_qty
                                else:
                                    exit_pnl = (entry_price - exit_price) * exit_qty
                        except Exception:
                            pass
                        live_rev_exit_placed = True
                except Exception as _e:
                    print(f"   🚨 Reverse LIVE exit failed for {symbol}: {_e}")
            ...
            exit_signal = {
                ...
                'trade': target_pos,
                'live_exit_placed': live_rev_exit_placed,
            }
            exit_pending = []
        ...
        # NFO: prefix for options, NSE: for equity. Equity reverse keeps same
        # symbol (just flips direction); only options swap PE↔CE strike.
        if is_option:
            nfo_symbol = f'NFO:{rev_symbol}' if not rev_symbol.startswith('NFO:') else rev_symbol
        else:
            nfo_symbol = rev_symbol if ':' in rev_symbol else f'NSE:{rev_symbol}'
        try:
        ...
        # ── LIVE MODE: place the new reverse-leg buy order synchronously ──
        if not PAPER_MODE:
            try:
                _exch_e, _tsym_e = nfo_symbol.split(':')
                # Reverse direction is captured in `direction` param. For an
                # equity reverse (LONG↔SHORT), flip the broker side. Options
                # default to BUY (long premium) since we flip PE↔CE.
                if is_option:
                    _tx_e = kite.TRANSACTION_TYPE_BUY
                else:
                    _tx_e = kite.TRANSACTION_TYPE_BUY if direction in ('BUY', 'LONG') else kite.TRANSACTION_TYPE_SELL
                _oid_e = _dashboard_place_order(
                    kite,
                    variety=kite.VARIETY_REGULAR,
                    exchange=_exch_e,
                    tradingsymbol=_tsym_e,
                    transaction_type=_tx_e,
                    quantity=quantity,
                    product=kite.PRODUCT_MIS,
                    order_type=kite.ORDER_TYPE_MARKET,
                    validity=kite.VALIDITY_DAY,
                    tag='TITAN_REVERSE_ENTRY',
                )
                paper_id = _oid_e  # persist real broker order_id
                try:
                    _fp_e = _get_completed_order_fill_price(kite, _oid_e)
                    if _fp_e > 0:
                        market_ltp = _fp_e
                except Exception:
                    pass
            except Exception as _e:
                print(f"   🚨 Reverse LIVE entry failed for {rev_symbol}: {_e}")
...
def add_lot():
    ...
    try:
        ...
        try:
            ...
            old_qty_raw = int(target_pos.get('quantity', 0) or 0)
            old_lots_raw = target_pos.get('lots')
            old_avg = float(target_pos.get('avg_price') or target_pos.get('entry_price') or 0)

            # Detect equity vs option position. Equity rows usually have no
            # `is_option` flag, no `option_type`, and an NSE: prefix.
            is_option_pos = bool(target_pos.get('is_option')) or bool(target_pos.get('option_type'))

            # Equity often has no `lots` field — treat the entire entry as
            # 5 sizing-chunks so +1 adds ~20% (matching the option default of
            # adding 1 of 5 lots). Options keep their explicit lot count.
            if is_option_pos:
                old_lots = int(old_lots_raw or 1)
            else:
                old_lots = int(old_lots_raw) if old_lots_raw else 5
            ...
            # Fetch live LTP via Kite — NFO: prefix for options, NSE:/MCX: for equity
            kite = _get_dashboard_kite()
            if not kite:
                return jsonify({'ok': False, 'msg': 'Kite API unavailable for add-lot'}), 503
            if is_option_pos:
                ltp_symbol = f'NFO:{symbol}' if not symbol.startswith('NFO:') else symbol
            else:
                # Equity — use the symbol as-is (already has NSE:/BSE:/MCX: prefix)
                ltp_symbol = symbol if (':' in symbol) else f'NSE:{symbol}'
            market_ltp = 0
            try:
                ltp_data = kite.ltp([ltp_symbol])
                if ltp_symbol in ltp_data:
                    market_ltp = float(ltp_data[ltp_symbol].get('last_price', 0) or 0)
            except Exception as e:
                print(f"⚠️ Add-lot LTP fetch failed for {ltp_symbol}: {e}")
            ...
            # ── LIVE MODE: place the add-lot BUY order synchronously so the
            #    user sees a confirmed broker fill in ~500ms, not 5–15s after
            #    the bot's next loop consumes the signal file. Mirrors the
            #    same direct-placement pattern used by /api/exit_trade.
            live_addlot_placed = False
            live_add_order_id = None
            if not PAPER_MODE:
                try:
                    _exch_a, _tsym_a = ltp_symbol.split(':')
                    live_add_order_id = _dashboard_place_order(
                        kite,
                        variety=kite.VARIETY_REGULAR,
                        exchange=_exch_a,
                        tradingsymbol=_tsym_a,
                    ...
                    # Use actual fill price for weighted-avg if available
                    try:
                        _fp_a = _get_completed_order_fill_price(kite, live_add_order_id)
                        if _fp_a > 0:
                            market_ltp = _fp_a
                    except Exception:
                        pass
                    live_addlot_placed = True
                except Exception as _e:
                    print(f"   🚨 Add-lot LIVE order failed for {symbol}: {_e}")
                    return jsonify({'ok': False, 'msg': f'Live add-lot order rejected: {_e}'}), 502
            new_qty = old_qty + add_qty
            new_lots = old_lots + add_lots
            new_avg = round(((old_avg * old_qty) + (market_ltp * add_qty)) / new_qty, 2)

            # SL/target recompute: option-specific bands (72%/160%) only apply to
            # long-option positions. For equity, preserve the original SL/target
            # (manager will trail them) — recomputing with option bands would be
            # disastrous on a stock.
            if is_option_pos:
                new_sl = round(new_avg * 0.72, 2)
                new_tgt = round(new_avg * 1.60, 2)
                new_max_loss = round((new_avg - new_sl) * new_qty, 2)
            else:
                new_sl = float(target_pos.get('stop_loss') or 0) or round(new_avg * 0.97, 2)
                new_tgt = float(target_pos.get('target') or 0) or round(new_avg * 1.05, 2)
                new_max_loss = round(abs(new_avg - new_sl) * new_qty, 2)
            ...
            pending.append({
                ...
                'live_addlot_placed': live_addlot_placed,
                'live_add_order_id': live_add_order_id,
                'new_qty': new_qty,
                'new_lots': new_lots,
                'new_avg': new_avg,
            ...
            _mode_tag = '[LIVE]' if live_addlot_placed else ('[PAPER]' if PAPER_MODE else '[QUEUED]')
            _broker_tag = f' | Broker order: {live_add_order_id}' if live_add_order_id else ''
            return jsonify({
                'ok': True,
                'msg': (f'{_mode_tag} Added 1 lot to {symbol} @ ₹{market_ltp:.2f} | '
                        f'Qty {old_qty}→{new_qty} | Lots {old_lots}→{new_lots} | '
                        f'Avg ₹{old_avg:.2f}→₹{new_avg:.2f} | '
                        f'SL ₹{new_sl:.2f} | TGT ₹{new_tgt:.2f}{_broker_tag}'),
                'ltp': market_ltp,
                'new_qty': new_qty,
                'new_lots': new_lots,
                'new_avg': new_avg,
            ...
            if kite:
                try:
                    ...
                    for leg_sym, leg_action in legs:
                        ...
                        order_id = _dashboard_place_order(
                            kite,
                            variety=kite.VARIETY_REGULAR,
                            exchange=exch,
                            tradingsymbol=tsym,
