...
from oi_watcher_engine import _oi_signal_from_result, _oi_direction, _OI_BEARISH_SIGNALS, _OI_BULLISH_SIGNALS, OIWatcherEngine
from watcher_pipeline import WatcherPipeline
from watcher_exit_engine import WatcherExitEngine
from capital_swap_engine import CapitalSwapEngine
...
class AutonomousTrader:
    ...
    def __init__(self, capital: float = 10000, paper_mode: bool = True):
        ...
        self._oi_watcher_min_strength = 0.20              # [FIX Mar 24 v2] was 0.15 — base strength floor before multipliers; rejects noise OI signals
        ...
        self._oi_min_confirmations = 5                     # [FIX Mar 23 v3] Minimum confirming factors to entry (was 4, raised for quality)
        self._oi_pending_confirm = {}                      # LEGACY — kept for OI_AGGR path
        self._oi_confirm_seconds = 25                      # [FIX Mar 24] Reduced 60→30→25s for faster entries
        ...
        self._oi_watcher_min_price_delta = 0.20           # Gate C mandatory: price must move ≥0.20% in OI direction
        ...
        self._oi_aggr_scan_interval = 70     # [FIX Mar 24] Reduced 90→70s for faster detection
        ...
        # Gate: OI_Watcher only runs after first scan_and_trade completes
        self._first_scan_completed = False

        # OI Watcher Engine (delegated from oi_watcher_engine.py)
        self._oi_engine = OIWatcherEngine(self)
        self._watcher_pipeline = WatcherPipeline(self)
        self._watcher_exit_engine = WatcherExitEngine(self)
        self._capital_swap_engine = CapitalSwapEngine(self)
        ...
        for sym, score, direction, data in elite_candidates[:max_fires]:
            ...
            try:
                ...
                _elite_ml_data = {
                    ...
                    'oi_strength': getattr(self, '_cycle_oi_results', {}).get(sym, {}).get('nse_oi_buildup_strength', None),
                } if _elite_ml else {}
                ...
                _elite_ml_data.setdefault('oi_strength', getattr(self, '_cycle_oi_results', {}).get(sym, {}).get('nse_oi_buildup_strength', None))
    ...
    def _find_eviction_candidate(self, incoming_setup_type: str) -> dict | None:
        """Find eviction candidate — delegated to capital_swap_engine.py"""
        return self._capital_swap_engine.find_eviction_candidate(incoming_setup_type)

    def _execute_eviction(self, candidate: dict, incoming_reason: str) -> bool:
        """Execute eviction — delegated to capital_swap_engine.py"""
        return self._capital_swap_engine.execute_eviction(candidate, incoming_reason)
    ...
    def _process_breakout_triggers(self):
        ...
        # ================================================================
        # OI_WATCHER — delegated to oi_watcher_engine.py
        # ================================================================
    ...
    def _aggressive_oi_buildup_scan(self):
        """Independent OI buildup scanner — delegated to oi_watcher_engine.py"""
    ...
    def _watcher_focused_scan(self, triggers: list):  # type: ignore[reportGeneralIssues]
        """Watcher focused scan pipeline — delegated to watcher_pipeline.py"""
        self._watcher_pipeline.focused_scan(triggers)

    # ========== WATCHER MOMENTUM EXIT (spike peaked / crater bottomed) ==========
    def _check_watcher_momentum_exits(self):
        """Check watcher momentum exits — delegated to watcher_exit_engine.py"""
        self._watcher_exit_engine.check_momentum_exits()
    ...
    def _proactive_loss_hedge_check(self):
        ...
        naked_opts = [
            ...
            and t.get('setup_type') != 'OI_WATCHER'  # OI_WATCHER: exit cleanly, no THP
        ]
        if not naked_opts:
            return
        ...
        while self.monitor_running:
            try:
                if self.is_trading_hours():
                    ...
                    # === OI_WATCHER AGGRESSIVE SCANNER (every 70s) ===
                    # Only runs after first scan_and_trade completes
                    if self._first_scan_completed:
    ...
    def _check_portfolio_profit_target(self):
        """KILL-ALL PROFIT SWITCH: Close ALL positions when unrealized P&L >= 12% of capital.
        
        Uses _compute_live_unrealized_pnl() for real-time unrealized P&L only.
        Realized P&L is NOT counted — only open position gains trigger the kill-all.
        ...
        target_pct = HARD_RULES.get('PORTFOLIO_PROFIT_TARGET', 0.12)
                ...
                if trade:
                    ...
                    if signal.exit_type.startswith('THESIS_INVALID_'):
                        ...
                        is_naked_option = (
                            ...
                            and trade.get('setup_type') != 'OI_WATCHER'  # OI_WATCHER: exit cleanly, no THP
                        )
                        if is_naked_option:
...
{symbol}:
  ...
  EOD Prediction: {eod_pred}"""
            ...
            try:
                ...
                # [FIX Mar 23 v3] Now uses sniper-expanded ML pool (ALL FNO stocks)
                # instead of just top-10 scored stocks. Same universe as snipers.
                try:
                    _tgmm_ml = getattr(self, '_sniper_expanded_ml', _ml_results)
                    _tgmm_scores = getattr(self, '_sniper_expanded_scores', _pre_scores)
                    self._place_test_gmm_trades(
                        _tgmm_ml, _tgmm_scores, market_data, datetime.now().strftime('%H:%M:%S')
                    )
...
RULES: F&O → place_option_order() | Cash → place_order() | Max {_dynamic_max} trades"""
    ...
    def run(self, scan_interval_minutes: int = 5):
        ...
        # Run loop
        _forced_0916_scan = False  # Track whether we've forced the 09:16 scan today
        _forced_0916_date = None   # Which date we did it for
        try:
            _loop_count = 0
            while True:
                # === FORCE FIRST SCAN AT 09:16 ===
                # The schedule timer may not align with market open.
                # Force a scan_and_trade() at 09:16 regardless of schedule.
                _now_force = datetime.now()
                _today_date = _now_force.date()
                if _forced_0916_date != _today_date:
                    _forced_0916_scan = False  # Reset for new day
                if (not _forced_0916_scan
                    and _now_force.hour == 9 and _now_force.minute >= 16
                    and self.is_trading_hours()):
                    _forced_0916_scan = True
                    _forced_0916_date = _today_date
                    _dbg("SCAN: ⚡ FORCED 09:16 scan_and_trade() — market just opened")
                    try:
                        self.scan_and_trade()
                        _dbg("SCAN: ⚡ FORCED 09:16 scan completed OK")
                    except Exception as _f916_e:
                        _dbg(f"ERROR: forced 09:16 scan CRASHED: {_f916_e}")
                        _dbg(f"TRACEBACK:\n{_tb.format_exc()}")
