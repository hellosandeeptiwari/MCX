        ...
        try:
            ...
            if _oi_futures:
                ...
                try:
                    ...
                    t._wlog(f"⚠️ OI background timeout — {_oi_done}/{_oi_total} fetched, proceeding with partial OI data")
                if _oi_executor:
            ...
            for _sym, _score in sorted(_pre_scores.items(), key=lambda x: x[1], reverse=True):
                ...
                if _trigger_type == 'PRICE_SPIKE_UP' and _scorer_dir:
                    if _scorer_dir != _trigger_dir:
                        t._wlog(f"🔄 SPIKE_BUY→SCORER: {_sym.replace('NSE:', '')} "
                                   f"spike_dir=BUY but scorer={_scorer_dir} → using scorer direction")
                    _trigger_dir = _scorer_dir
                ...
                _score_audit = _cycle_decisions.get(_sym, {}).get('audit', '')
                _stock_name = _sym.replace('NSE:', '')
