"""
TITAN v5 ΓÇö Monitoring Dashboard
================================
...
# ΓöÇΓöÇ Lightweight Kite instance for LIVE exit orders ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def _get_dashboard_kite():
    ...
    try:
        ...
        print(f"ΓÜá∩╕Å Dashboard KiteConnect init failed: {e}")
        return None
...
# ΓöÇΓöÇ App setup ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Server-Sent Events: live log tail ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def _sse_log_stream(filepath: str):
    ...
    - 100ms poll instead of 500ms ΓåÆ 5x faster log delivery
    - SSE heartbeat comment every 15s ΓåÆ keeps connection alive through
            ...
            while True:
                ...
                if line:
                    ...
                    try:
                        ...
                        if cur_size < where:
                            # File was rotated ΓÇö reopen from start
                            f.seek(0)
                            continue
                    except OSError:
                        pass
...
# Server boot timestamp ΓÇö changes on every restart (i.e. every deploy)
...
# ΓöÇΓöÇ Live log SSE streams ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Smart Log Reader with Bookmarks ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def smart_logs():
    ...
    Query params:
      file          ΓÇô log key: titan|titan_error|dashboard_error|watchdog|bot_debug
                      (default: titan). Use "all" for metadata-only overview.
      lines         ΓÇô max lines to return (default: 300)
      since_bookmark ΓÇô if "true", return only new lines after saved bookmark
      set_bookmark  ΓÇô if "true", save current end-of-file as bookmark after read
      from_scan     ΓÇô if "true", return lines starting from last SCAN CYCLE
      grep          ΓÇô optional regex filter applied to returned lines
      tail          ΓÇô if "true" (default), read last N lines; if "false", read from bookmark/scan
    ...
    # ΓöÇΓöÇ Always return metadata for ALL log files ΓöÇΓöÇ
    meta = {}
    ...
    # ΓöÇΓöÇ If "all", return just metadata overview ΓöÇΓöÇ
    if file_key == 'all':
        if set_bm:
    ...
    # ΓöÇΓöÇ Decide start line ΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Optional grep filter ΓöÇΓöÇ
    if grep_pat:
        try:
            pat = _re.compile(grep_pat, _re.IGNORECASE)
            lines = [l for l in lines if pat.search(l)]
        except _re.error:
            pass  # bad regex ΓÇö return unfiltered

    # ΓöÇΓöÇ Set bookmark ΓöÇΓöÇ
    bm_now = None
    if set_bm:
        bm_now = total
...
# ΓöÇΓöÇ Bot control (start / stop / restart) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Manual position exit (dashboard ΓåÆ signal file ΓåÆ bot) ΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def exit_position():
    ...
    try:
        ...
        # For PE options: direction=SELL but side=BUY ΓåÆ must use side for correct P&L
        ...
        # 1∩╕ÅΓâú  LIVE MODE: Place real exit order immediately from dashboard
        ...
        if not PAPER_MODE:
            ...
            if kite:
                try:
                    ...
                    live_exit_msg = f'ΓÜá∩╕Å LIVE exit order failed: {e}'
                    print(f"   ≡ƒÜ¿ Dashboard LIVE exit failed for {symbol}: {e}")

        # 2∩╕ÅΓâú  Write signal file for bot (in-memory cleanup + sync)
        signal = {
            'symbol': symbol,
        ...
        try:
            ...
            print(f"ΓÜá∩╕Å Trade ledger log failed for manual exit: {e}")

        _mode_label = 'LIVE' if not PAPER_MODE else 'PAPER'
        _exit_msg = f'[{_mode_label}] Exited {symbol} @ Γé╣{exit_price:.2f} | P&L: Γé╣{pnl:+,.2f}'
        if live_exit_placed:
            _exit_msg += ' | Broker order placed Γ£à'
        elif not PAPER_MODE:
...
def exit_all_positions():
    """Exit ALL open positions at market price ΓÇö emergency kill switch.
    ...
    try:
        ...
        for pos in open_positions:
            ...
            # P&L calculation ΓÇö same logic as single exit
            ...
            try:
                ...
                print(f"ΓÜá∩╕Å Trade ledger log failed (exit_all) for {symbol}: {e}")

            total_pnl += pnl
            status = 'Γ£à' if live_exit_placed or PAPER_MODE else f'ΓÜá∩╕Å {live_exit_msg}'
        ...
        msg = f'[{_mode}] Exited {len(results)} positions | Net P&L: Γé╣{total_pnl:+,.2f}'
        print(f"≡ƒÜ¿ EXIT ALL: {msg}")
...
# ΓöÇΓöÇ Helpers: enrich positions with exit-state LTP & unrealized P&L ΓöÇΓöÇ
...
# ΓöÇΓöÇ System status ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def get_status():
    ...
    # Override realized_pnl with ledger truth (survives bot restarts).
    # Falls back to in-memory counter if ledger read fails.
    try:
        _ledger_summary = get_trade_ledger().daily_summary(today)
        if _ledger_summary.get('total_trades', 0) > 0:
            realized_pnl = _ledger_summary.get('total_pnl', realized_pnl)
    except Exception:
        pass
...
# ΓöÇΓöÇ Trade ledger ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

@app.route('/api/trade_summary')
def trade_summary():
    """One-call comprehensive trade summary for today.
    Includes: positions, P&L, ledger events, risk state ΓÇö everything."""
    import re as _re
    db = get_state_db()
    today = _today()
    ...
    # ΓöÇΓöÇ Enrich open positions ΓöÇΓöÇ
    open_positions = []
    total_unreal = 0
    for p in positions:
        sym = p.get('symbol', '')
    ...
    # ΓöÇΓöÇ Risk state ΓöÇΓöÇ
    risk = db.load_risk_state(today) or {}

    # ΓöÇΓöÇ Build summary ΓöÇΓöÇ
...
# ΓöÇΓöÇ Scan decisions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
@app.route('/api/scans')
def scan_decisions():
    db = get_state_db()
...
# ΓöÇΓöÇ Slippage log ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Orders ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Exit states ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
@app.route('/api/exits')
def exit_states():
    db = get_state_db()
...
# ΓöÇΓöÇ Ledger dates ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ P&L calendar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Config ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  SETTINGS ΓÇö Single source of truth via settings_manager
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
def kill_switch():
    ...
    try:
        ...
        return jsonify({'ok': True, 'msg': '≡ƒÜ¿ KILL SWITCH ACTIVATED ΓÇö Bot stopped, all trading halted.'})
    except Exception as e:
...
# ΓöÇΓöÇ Pre-market health check ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def health_check():
    ...
    try:
        ...
        passed = sum(1 for l in lines if 'Γ£à' in l)
        warned = sum(1 for l in lines if 'ΓÜá' in l)
        failed = sum(1 for l in lines if 'Γ¥î' in l)
        return jsonify({
            'ok': failed == 0,
            'passed': passed,
            'warned': warned,
...
# ΓöÇΓöÇ Watchdog alerts ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def run_dashboard(host='0.0.0.0', port=5000, debug=False):
    print(f"\n{'='*56}")
    print(f"  TITAN v5 ΓÇö Monitoring Dashboard")
